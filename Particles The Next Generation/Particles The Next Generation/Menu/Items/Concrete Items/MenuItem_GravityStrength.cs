...
{
    class MenuItem_GravityStrength : MenuItemBar
    {
        protected Physics_System m_System;
        protected Checkbox m_Checkbox;

        public MenuItem_GravityStrength(Vector2 position, Texture2D selectedSprite, Texture2D deselectedSprite, Physics_System obj, float min, float max, float current)
            : base(position, selectedSprite, deselectedSprite, obj, min, max, current)
        {
            ...
            this.m_Marker = Utility.Load<Texture2D>("Menu/Items/Gravity/gravitystrength_marker");
